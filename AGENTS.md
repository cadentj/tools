...
**docs**: Prefer `update replace` for edits (no index bookkeeping). Run `get` before positional edits; re-`get` after (indices shift). Edit bottom-to-top for multiple index edits. Run `tabs` first if tab names are unknown.
